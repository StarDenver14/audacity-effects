...
;type process
;name "Ghost Voice"
;action "Applying ghost voice effect..."
;author "Audacity Plugin"
;copyright "Public Domain"
;info "Whispery, ethereal voice with rumble."

;control intensity "Effect Intensity" real "%" 50 0 100
;control whisper "Whisper Base" real "" 0.6 0 1
;control reverse "Reverse Reverb" real "" 0.5 0 1
;control rumble "Low Rumble" real "" 0.3 0 1
...
(let* ((t (/ intensity 100.0))
       (breath (mult (highpass2 (noise (get-duration 1)) 4000) (* t whisper)))
       (pre (mult (delay *track* 0.06) (* t reverse)))
       (rum (mult (osc (hz-to-step 50)) (* t rumble 0.2)))
       (wet (sum *track* breath pre rum))
       (out (mix *track* wet t)))
  out)
