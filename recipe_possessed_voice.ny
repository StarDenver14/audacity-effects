...
;version 4
;type process
;name "Possessed Voice Recipe"
;action "Applying possessed voice recipe effect..."
;author "Audacity Plugin"
;copyright "Public Domain"
;info "Monster, dizzy, and through-wall combined."
...
(let* ((t (/ intensity 100.0))
       (pitch (pow 2 (/ (* t -8) 12.0)))
       (pitched (resample *track* pitch))
       (distorted (soft-clip pitched (* t comp1)))
       (wob (sum 1 (mult (* t comp2) (osc (hz-to-step 0.3)))))
       (wobbly (mult distorted wob))
       (muffled (lowpass2 wobbly (lerp 20000 1500 (* t comp3))))
       (out (mix *track* muffled t)))
  out)
