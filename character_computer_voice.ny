...
;type process
;name "Computer Voice"
;action "Applying computer voice effect..."
;author "Audacity Plugin"
;copyright "Public Domain"
;info "Clean, precise synthetic voice with subtle chorus."

;control intensity "Effect Intensity" real "%" 50 0 100
;control perfect "Perfect Pitch" real "" 0.8 0 1
;control nobreath "No Breath" real "" 0.9 0 1
;control chorus "Slight Chorus" real "" 0.2 0 1
...
(let* ((t (/ intensity 100.0))
       (clean (lowpass2 *track* (lerp 20000 6000 (* t nobreath))))
       (ratio (pow 2 (/ (* t chorus) 12.0)))
       (layer (resample clean ratio))
       (mixed (sum clean (mult layer (* t chorus))))
       (steady (soft-clip mixed (* t perfect 0.2)))
       (out (mix *track* steady t)))
  out)
